...
The critic receives all three prior outputs and checks for internal contradictions, unsupported assumptions, missing risks, and methodology weaknesses
If it finds issues, it returns REVISE: <specific reason> — the graph routes back to the valuation agent with the critic's feedback injected into the prompt
The valuation agent explicitly addresses the flagged concerns in its revised output
This repeats up to 2 times (hard-capped in the conditional edge, not just by prompt instruction)
